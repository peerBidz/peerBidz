class BiddingsController < ApplicationController
  def create
    ...
    if @bidding.save
      ...
      if (!@old_highest_bid.nil? && @old_highest_bid.ipaddress != @bidding.ipaddress)
      #Notification for the user who was outbid
      @msg = "You have been outbid on: " + Item.find(@bidding.item_id).title
      Notification.create(:item_id=>@bidding.item_id, :message=>@msg, :delivered=>"false", :notification_type => 'O')
      end
    ...
    Notification.create(:item_id=>@bidding.item_id, :message=>@msg, :delivered=>"false", :notification_type => 'N')
